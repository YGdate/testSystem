  ...
  v-model="zw">
    ...
    <div class="con-right">
         <p class="headp">
            ...
            <div class="tkt" v-for="(item,i) in allmsg.text_mistake" :key="i+7000">
            <p>短文改错第{{i+1}}题</p>
            <el-input
  type="textarea"
  :rows="2"
...
solve.single_select=[]
   ...
   solve.seven_selected_five=[]
            for(let x=0;x<this.allmsg.seven_selected_five.length;x++){
                let y = {}
                y.id = this.allmsg.seven_selected_five[x].id;
                y.ans = {};
 ...
 solve.non_directional_select=[]
     ...
     solve.text_mistake=[]
         ...
         this.$router.push("testpage");
                ...
                let once=0;
                if(msg.data.msg=="当前考试已结束！"&&once==0){
                     this.$message.error(msg.data.msg)
                     once++;
                }else if (once==0){
                    this.$message.error("抱歉，您的试卷提交失败！")
                    once++;
                }
