<template>
    ...
    <div class="ingcon">
        ...
        <div class="con-left">
            ...
            <ul class="qc">
                <li :class="tab==tabs[0]?'lactive':''" data-aid="1" @click="qian($event.srcElement.dataset.aid)">单选题
                </li>
                <li :class="tab==tabs[1]?'lactive':''" data-aid="2" @click="qian($event.srcElement.dataset.aid)">多选题
                </li>
                <li :class="tab==tabs[2]?'lactive':''" data-aid="3" @click="qian($event.srcElement.dataset.aid)">七选五
                </li>
                <li :class="tab==tabs[3]?'lactive':''" data-aid="4" @click="qian($event.srcElement.dataset.aid)">判断题
                </li>
                <li :class="tab==tabs[4]?'lactive':''" data-aid="5" @click="qian($event.srcElement.dataset.aid)">填空题
                </li>
                <li :class="tab==tabs[5]?'lactive':''" data-aid="6" @click="qian($event.srcElement.dataset.aid)">选词填空
                </li>
                <li :class="tab==tabs[6]?'lactive':''" data-aid="7" @click="qian($event.srcElement.dataset.aid)">完型填空
                </li>
                <li :class="tab==tabs[7]?'lactive':''" data-aid="8" @click="qian($event.srcElement.dataset.aid)">听力</li>
                <li :class="tab==tabs[8]?'lactive':''" data-aid="9" @click="qian($event.srcElement.dataset.aid)">非定向选择
                </li>
                <li :class="tab==tabs[9]?'lactive':''" data-aid="10" @click="qian($event.srcElement.dataset.aid)">阅读理解
                </li>
                <li :class="tab==tabs[10]?'lactive':''" data-aid="11" @click="qian($event.srcElement.dataset.aid)">短文改错
                </li>
                <li :class="tab==tabs[11]?'lactive':''" data-aid="12" @click="qian($event.srcElement.dataset.aid)">翻译
                </li>
                <li :class="tab==tabs[12]?'lactive':''" data-aid="13" @click="qian($event.srcElement.dataset.aid)">作文
                </li>
            </ul>
        </div>
        <div class="con-center">
            ...
            <div class="xctks" :class="tab!=tabs[5]?'tt':''">
                ...
                <div clas="xctk zong" v-for="(item,i) in allmsg.choose_fill_blank" :key="item.id">
                    ...
                    <div v-for="(item, key, x) in allmsg.choose_fill_blank[i].topic_and_stem.options" :key="x">
                        {{key}}:{{item}}</div>
                    <el-input v-model="xuan[i][y]" maxlength=1 style="width:50px;" v-for="(itm,y) in xuan[i]"
                        :key="y+2000" :placeholder="y+1" @input="updateView($event)"></el-input>
                </div>
            </div>
            ...
            <div class="tinglis" :class="tab!=tabs[7]?'tt':''">
                ...
                <div v-for="(item2,i2) in allmsg.listening" :key="item2.id">
                    ...
                    <div class="tingli zong" v-for="(item,i) in allmsg.listening[0].topic_and_stem.title" :key="i+4000"
            ...
            <div class="fdxs" :class="tab!=tabs[8]?'tt':''">
                ...
                <div class="fdx zong" v-for="(item,i) in allmsg.non_directional_select" :key="item.id">
                    <p>{{i+1}}.{{item.topic_and_stem.title}}</p>
                    <template>
                        <el-checkbox v-model="fei[i]" label="A" class="xuanze">{{item.topic_and_stem.options.A}}
                        </el-checkbox>
                        <el-checkbox v-model="fei[i]" label="B" class="xuanze">{{item.topic_and_stem.options.B}}
                        </el-checkbox>
                        <el-checkbox v-model="fei[i]" label="C" class="xuanze">{{item.topic_and_stem.options.C}}
                        </el-checkbox>
                        <el-checkbox v-model="fei[i]" label="D" class="xuanze">{{item.topic_and_stem.options.D}}
                        </el-checkbox>
                    </template>
                </div>
            </div>
    ...
    export default {
        ...
        methods: {
            ...
            gettime: function () {
                ...
                let time = storage.getItem("examtime");
                this.alltime = time;
                console.log(time)
                this.fen = parseInt(this.alltime / 60);
                this.miao = this.alltime % 60;
            },
            close: function () {
                this.sureclose = true;
            },
                ...
                if (msg.data.code == 0) {
                    ...
                    if (xf.choose_fill_blank) {
                        for (let i = 0; i < this.allmsg.choose_fill_blank.length; i++) {
                            this.xuan[i] = [];
                            this.allmsg.choose_fill_blank[i].topic_and_stem = JSON.parse(this.allmsg
                                .choose_fill_blank[i].topic_and_stem);
                            for (let n = 0; n < 10; n++) {
                                this.xuan[i][n] = '';
                            }
                        }
                    ...
                    if (xf.read_understand) {
                        for (let i = 0; i < this.allmsg.read_understand.length; i++) {
                            this.yuedu[i] = [];
                            let m = 0;
                            this.allmsg.read_understand[i].topic_and_stem = JSON.parse(this.allmsg.read_understand[
                                i].topic_and_stem)
