...
console.log(ms)
 ...
 let x = ms.data;
     ...
     for(let i=0;i<x.length;i++){
       if(x[i].exam_time_log==null){
         x[i].panduan=true;
       }else{
         let p = x[i].exam_time_log.test_end_at;
          let d = new Date();
    ...
    let curTime = year + '-' + mouth + '-' + day + ' ' + hour + ':' + minutes + ':' + second;
    //console.log(p,curTime)
        let dt = new Date(p).getTime();
let nt = new Date(curTime).getTime();
if(dt>nt||x[i].exam_time_log.user_start_at==null){
   x[i].panduan=true;
}else{
   x[i].panduan=false;
}
  ...
  tanchu:function(){
    this.$message.error("抱歉该考试已经结束！")
  },
  // 去考试
  goexam:function(e,time){
    let ttime = 0;
    ttime = ttime+time[0]*10*60*60;
     ttime = ttime+time[1]*60*60;
      ttime = ttime+time[3]*10*60;
       ttime = ttime+time[4]*60;
        ttime = ttime+time[6]*10;
        let ge = parseInt(time[7]);
        
          ttime = ttime+ge;
   var storage = window.sessionStorage;  
     storage.setItem('examid', e);
     storage.setItem('examtime', ttime);
