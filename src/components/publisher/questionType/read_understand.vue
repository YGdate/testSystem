<template>
  ...
  <el-row class="question-content">
    ...
    <el-row class="questionSelect" v-for="(item1,index1) in question" :key="index1">
      ...
      <el-row>
        <el-col style="margin-bottom:20px;"><span style="font-size:16px;font-weight:600">Question{{item1.name}}</span>
    ...
    methods: {
      dataComputed() {
        ...
        for (let item in testData.options) {
          let newData = []
          for(let item1 in testData.options[item].options){
            newData.push({
              value:testData.options[item].options[item1],
              name:item1
            }) 
          }
          newOption.push({
            name: item,
            title:testData.options[item].title,
            value: newData
          })
        }
